...
controlGroup= Cgroup('{varHostName}')
...
def cpu_cgroup(args):
    controlGroup.set_cpu_limit(50)
    pass

def mem_cgroup(args):
    controlGroup.set_memory_limit(10)
    pass
...
def exe_bash(args):
    ...
    if newpid == 0:
        varHostName = args.hostname   
        os.system(f'hostname {varHostName}')
        pid=os.getpid()
        controlGroup.add(pid)
        set_eth1()
        os.system("ifconfig eth1 10.0.0.1")
